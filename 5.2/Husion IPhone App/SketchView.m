...
-(void) moveLabel{
    [self moveRefreshLabelBack];
    [self performSelector:@selector(MoveRefreshLabelAway) withObject:nil afterDelay:1.00];
    //[self performSelector:@selector(moveRefreshLabelBack) withObject:nil afterDelay:2.00];    
    //NSLog(@"label position y: %f", refreshRemindLabel.frame.origin.y);

}

-(void) moveLabel2{
    [self moveMapUpdatedlBack];
    [self performSelector:@selector(MoveMapUpdatedLabelAway) withObject:nil afterDelay:1.00];
    //[self performSelector:@selector(moveMapUpdatedlBack) withObject:nil afterDelay:2.00];     
